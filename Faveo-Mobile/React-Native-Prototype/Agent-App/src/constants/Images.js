export const Logo = require("../assets/images/logo-1.png");
export const UrlBG = require("../assets/images/splash-screen-2.png");
export const BackArrow = require("../assets/images/back-arrow.png");
export const ChangeLanguage = require("../assets/images/changelanguage.png");
export const Check = require("../assets/images/check.png");
export const Avatar = require("../assets/images/avatar-img.png");
export const SplashBG3 = require("../assets/images/splash-screen-3.png");
export const SplashBG1 = require("../assets/images/splash-screen1.png");
export const Splash1 = require("../assets/images/splash-img-1.png");
export const Splash2 = require("../assets/images/splash-img-2.png");
export const Splash3 = require("../assets/images/splash-img-3.png");
export const NextArrow = require("../assets/images/next-arrow.png");
export const Menu = require("../assets/images/menu.png");
export const Search = require("../assets/images/search.png");
export const Sort = require("../assets/images/sort.png");
export const Filter = require("../assets/images/filter.png");
export const Mail = require("../assets/images/mail.png");
export const UserTicket = require("../assets/images/Ellipse56.png");
export const AddUser = require("../assets/images/add-user.png");
export const OpenArrow = require("../assets/images/open-arrow.png");
export const Contact = require("../assets/images/contact.png");
export const Attachment = require("../assets/images/attachment.png");
export const Forword = require("../assets/images/forword.png");
export const BackWhite = require("../assets/images/back-w.png");
export const bprofile = require("../assets/images/Group118.png");
export const rprofile = require("../assets/images/Group351.png");
export const user = require("../assets/images/user.png");
export const edit = require("../assets/images/edit.png");
export const ticketNotification = require("../assets/images/ticketNotification.png");
export const notification = require("../assets/images/notificationSchedule.png");
export const notificationSchedule = require("../assets/images/ticketNotification.png");
export const about = require("../assets/images/about.png");
export const appLock = require("../assets/images/appLock.png");
export const give = require("../assets/images/giveFeedback.png");
export const signOut = require("../assets/images/signOut.png");
export const rateApp = require("../assets/images/rateApp.png");
export const whatnew = require("../assets/images/whatnew.png");
export const redAlert = require("../assets/images/alertred.png");
export const clanguage = require("../assets/images/clanguage.png");
export const file = require("../assets/images/file.png");
export const attach = require("../assets/images/attach.png");
export const cross = require("../assets/images/cross.png");
export const addcontact = require("../assets/images/addcontact.png");
export const wedit = require("../assets/images/wedit.png");
export const PLUS = require("../assets/images/plus.png");
export const Service = require("../assets/images/Crosscontact.png");
export const CALL = require("../assets/images/CALL.png");
export const createTicket = require("../assets/images/ticket1.png");
export const Inbox = require("../assets/images/inbox.png");
export const count12 = require("../assets/images/count12.png");
export const Ellipse8 = require("../assets/images/Ellipse8.png");
export const logout = require("../assets/images/logout1.png");
export const trash = require("../assets/images/trash.png");
export const ticket1 = require("../assets/images/ticket1.png");
export const ticket3 = require("../assets/images/ticket3.png");
export const ticket4 = require("../assets/images/ticket4.png");
export const ticket5 = require("../assets/images/ticket5.png");
export const ticket6 = require("../assets/images/ticket6.png");
export const ticket7 = require("../assets/images/ticket7.png");
export const ticket8 = require("../assets/images/ticket8.png");
export const abouts = require("../assets/images/abouts.png");
export const homeTabActive = require("../assets/images/home-tab-active.png");
export const homeTab = require("../assets/images/home-tab.png");
export const contactsTab = require("../assets/images/contacts-tab.png");
export const contactsTabActive = require("../assets/images/contacts-tab-active.png");
export const analyticsTab = require("../assets/images/analytics-tab.png");
export const analyticsTabActive = require("../assets/images/analytics-tab-active.png");
export const notificationTab = require("../assets/images/notification-tab.png");
export const notificationTabActive = require("../assets/images/notification-tab-active.png");
export const accountTab = require("../assets/images/account-tab.png");
export const accountTabActive = require("../assets/images/account-tab-active.png");
export const Notifications = require("../assets/images/Notifications.png");
export const Analytics = require("../assets/images/Analytics.png");
export const threedot = require("../assets/images/3dot.png");
export const noLogs = require("../assets/images/no-logs.png");
export const Backward = require("../assets/images/backward.png");
export const Calender = require("../assets/images/Calender.png");
export const PlusWhite = require("../assets/images/plus-white.png");
export const AddIcon = require("../assets/images/add-icon.png");
export const DownArrow = require("../assets/images/down-arrow.png");
export const AttachIcon = require("../assets/images/attachIcon.png");
export const NotifyCustomer = require("../assets/images/notify-customer.png");
export const Group5 = require("../assets/images/Group5.png");
export const Group464 = require("../assets/images/Group464.png");
export const PlusIcon = require("../assets/images/plus-icon.png");
export const Reply = require("../assets/images/reply.png");
export const Forward = require("../assets/images/Forward.png");
export const AddNote = require("../assets/images/Add-Note.png");
export const GroupClose = require("../assets/images/Group-close.png");
export const GroupScenario = require("../assets/images/Group-Scenerio.png");
export const LogTime = require("../assets/images/Group-log-time.png");
export const GroupMerge = require("../assets/images/Group-Merge.png");
export const GroupDueDate = require("../assets/images/Group-due-date.png");
export const Email = require("../assets/images/email.png");
export const Watchers = require("../assets/images/watcher.png");
export const ReplyGray = require("../assets/images/reply-gray.png");
export const AddNoteGray = require("../assets/images/add-note-gray.png");
export const SendIcon = require("../assets/images/Send.png");
export const CrossWhite = require("../assets/images/cross-white.png");
export const NewContact = require("../assets/images/new-contact.png");
export const NewEmail = require("../assets/images/new-email.png");
export const NewServiceTask = require("../assets/images/new-service-task.png");
export const NewTicket = require("../assets/images/new-ticket.png");
export const noTickets = require("../assets/images/no-tickets.png");
export const deleteMinus = require("../assets/images/delete-minus.png");
export const SearchIllustration = require("../assets/images/Search-tickets.png");
export const CrossCircleWhite = require("../assets/images/cross-circle-white.png");
export const FileUpload = require("../assets/images/file-upload.png");

export default {
  Logo,
  UrlBG,
  BackArrow,
  ChangeLanguage,
  Check,
  Avatar,
  SplashBG3,
  SplashBG1,
  Splash1,
  Splash2,
  Splash3,
  NextArrow,
  Menu,
  Search,
  Sort,
  Filter,
  UserTicket,
  Mail,
  AddUser,
  OpenArrow,
  Contact,
  Attachment,
  Forword,
  BackWhite,
  bprofile,
  rprofile,
  user,
  edit,
  ticketNotification,
  notification,
  notificationSchedule,
  about,
  appLock,
  give,
  signOut,
  rateApp,
  whatnew,
  redAlert,
  clanguage,
  file,
  cross,
  attach,
  addcontact,
  wedit,
  PLUS,
  Service,
  CALL,
  createTicket,
  Inbox,
  count12,
  Ellipse8,
  logout,
  trash,
  ticket1,
  ticket3,
  ticket4,
  ticket5,
  ticket6,
  ticket7,
  ticket8,
  abouts,
  homeTabActive,
  homeTab,
  contactsTab,
  contactsTabActive,
  analyticsTab,
  analyticsTabActive,
  notificationTab,
  notificationTabActive,
  accountTab,
  accountTabActive,
  Notifications,
  Analytics,
  threedot,
  noLogs,
  Backward,
  Calender,
  PlusWhite,
  AddIcon,
  DownArrow,
  AttachIcon,
  NotifyCustomer,
  Group5,
  Group464,
  PlusIcon,
  Reply,
  Forward,
  AddNote,
  GroupClose,
  GroupScenario,
  LogTime,
  GroupMerge,
  GroupDueDate,
  Email,
  Watchers,
  ReplyGray,
  AddNoteGray,
  SendIcon,
  CrossWhite,
  NewContact,
  NewEmail,
  NewServiceTask,
  NewTicket,
  noTickets,
  deleteMinus,
  SearchIllustration,
  CrossCircleWhite,
  FileUpload,
};
